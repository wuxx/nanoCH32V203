...
//    pRX_LEN_REG = (PUINT16)&R32_RX_LEN;  /* 指向接收长度寄存器,由应用程序初始化 */
	  pTX_DMA_A_REG = (PUINT32)&(USBOTG_H_FS->HOST_TX_DMA);  /* 指向发送DMA地址寄存器,由应用程序初始化 */
	  pRX_DMA_A_REG = (PUINT32)&(USBOTG_H_FS->HOST_RX_DMA);  /* 指向接收DMA地址寄存器,由应用程序初始化 */
	  pTX_LEN_REG = (PUINT16)&(USBOTG_H_FS->HOST_TX_LEN);  /* 指向发送长度寄存器,由应用程序初始化 */
	  pRX_LEN_REG = (PUINT16)&(USBOTG_H_FS->RX_LEN);  /* 指向接收长度寄存器,由应用程序初始化 */
