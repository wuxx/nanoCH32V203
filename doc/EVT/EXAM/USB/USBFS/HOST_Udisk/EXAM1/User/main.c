...
__attribute__ ((aligned(16))) UINT8   buf[ 128 ];  // OUT, must even address
                    ...
                    for ( i = 0; i != 10; i ++ )
                    {
                          ...
                          if ( ret == ERR_SUCCESS )
                          {
                              ...
                              if ( ret == ERR_MISS_DIR || ret == ERR_MISS_FILE )            //没有找到文件
                              {
                                  ...
                                  strcpy( (PCHAR)mCmdParam.Create.mPathName, "/C51/NEWFILE.C" );          /* 新文件名,在根目录下,中文文件名 */
