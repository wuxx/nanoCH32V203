...
__attribute__ ((aligned(4))) const UINT8  SetupSetUsbConfig[] = { USB_REQ_TYP_OUT, USB_SET_CONFIGURATION, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00 };
...
__attribute__ ((aligned(4))) UINT8   RxBuffer[ 128 ];  // IN, must even address
__attribute__ ((aligned(4))) UINT8   TxBuffer[ 128 ];  // OUT, must even address

#define pSetupReq    ((PUSB_SETUP_REQ)TxBuffer)

UINT32  HOST_TX_RAM_Addr;
UINT32  HOST_RX_RAM_Addr;
...
#if 0
        USBHD_UH_RX_DMA = HOST_RX_RAM_Addr;                       //host rx DMA address
        USBHD_UH_TX_DMA = HOST_TX_RAM_Addr;                       //host tx DMA address
#else
        USBHD_UH_RX_DMA= (UINT32)&RxBuffer[0];                  //host rx DMA address
        USBHD_UH_TX_DMA = (UINT32)&TxBuffer[0];                 //host tx DMA address
#endif
...
UINT8 USBOTG_HostEnum( UINT8 *buf )
{
    ...
    dv_cls = ( (PUSB_DEV_DESCR)buf ) -> bDeviceClass;
    ...
    if_cls = ( (PUSB_CFG_DESCR_LONG)buf )->itf_descr.bInterfaceClass;
    pOTGHost->DeviceCongValue = ( (PUSB_CFG_DESCR_LONG)buf )->cfg_descr.iConfiguration;
    if ( (dv_cls == 0x00) && (if_cls == USB_DEV_CLASS_STORAGE))
    {
        s = CtrlSetUsbConfig(  pOTGHost->DeviceCongValue );
        if( s == ERR_SUCCESS ){
            pOTGHost->DeviceType = USB_DEV_CLASS_STORAGE;
            return( ERR_SUCCESS );
        }
    }
    return( s );
    ...
    {
       printf("get configuration descriptor error\n");
       pOTGHost->DeviceCongValue = buf[5];
    }
