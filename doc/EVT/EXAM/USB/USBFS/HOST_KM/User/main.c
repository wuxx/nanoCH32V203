        ...
        while(flg)
        {
            ...
            if ( USBOTG_H_FS->INT_FG & USBHD_UIF_DETECT )
            {
                ...
                if( s == ERR_USB_DISCON )
                {
                    ...
                    USBOTG_H_FS->HOST_TX_DMA = (UINT32)&endpRXbuff[0];                  //host rx DMA address;
                }
            }
                    ...
                    for( i=0; i<HostCtl[0].InterfaceNum; i++ )
                    {
                        for( j=0; j<HostCtl[0].Interface[i].InEndpNum; j++ )
                        {
                            ...
                            if( HostCtl[0].Interface[i].InEndpTimeCount[j] >= HostCtl[0].Interface[i].InEndpInterval[j] )
                            {
                                ...
                                /* 使用单独的缓冲区地址 */
                                USBHD_UH_RX_DMA= (UINT32)&keybufdata[0];                  //host rx DMA address
                                ...
                                if ( s == ERR_SUCCESS )
                                {
                                    ...
                                    if( USBOTG_FS->RX_LEN > 0 )
                                    {
                                        printf( "Recv " );
                                        for( k=0; k<USBOTG_FS->RX_LEN; k++ )
                                        {
