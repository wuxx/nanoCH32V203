...
const UINT8 *pDescr;
volatile UINT8  USBHD_Dev_SetupReqCode = 0xFF;                                  /* USBOTG设备Setup包命令码 */
volatile UINT16 USBHD_Dev_SetupReqLen = 0x00;                                   /* USBOTG设备Setup包长度 */
volatile UINT8  USBHD_Dev_SetupReqValueH = 0x00;                                /* USBOTG设备Setup包Value高字节 */
volatile UINT8  USBHD_Dev_Config = 0x00;                                        /* USBOTG设备配置值 */
volatile UINT8  USBHD_Dev_Address = 0x00;                                       /* USBOTG设备地址值 */
volatile UINT8  USBHD_Dev_SleepStatus = 0x00;                                   /* USBOTG设备睡眠状态 */
volatile UINT8  USBHD_Dev_EnumStatus = 0x00;                                    /* USBOTG设备枚举状态 */
volatile UINT8  USBHD_Dev_Endp0_Tog = 0x01;                                     /* USBOTG设备端点0同步标志 */
volatile UINT8  USBHD_Dev_Speed = 0x01;                                         /* USBOTG设备速度 */

volatile UINT16 USBHD_Endp1_Up_Flag = 0x00;                                     /* USBOTG设备端点1数据上传状态: 0:空闲; 1:正在上传; */
volatile UINT8  USBHD_Endp1_Down_Flag = 0x00;                                   /* USBOTG设备端点1下传成功标志 */
volatile UINT8  USBHD_Endp1_Down_Len = 0x00;                                    /* USBOTG设备端点1下传长度 */
volatile BOOL   USBHD_Endp1_T_Tog = 0;                                          /* USBOTG设备端点1tog位翻转 */
volatile BOOL   USBHD_Endp1_R_Tog = 0;

volatile UINT16 USBHD_Endp2_Up_Flag = 0x00;                                     /* USBOTG设备端点2数据上传状态: 0:空闲; 1:正在上传; */
volatile UINT16 USBHD_Endp2_Up_LoadPtr = 0x00;                                  /* USBOTG设备端点2数据上传装载偏移 */
volatile UINT8  USBHD_Endp2_Down_Flag = 0x00;                                   /* USBOTG设备端点2下传成功标志 */
volatile BOOL   USBHD_Endp2_T_Tog = 0;                                          /* USBOTG设备端点2tog位翻转 */
...
UINT8  MyCfgDescr_LS[ ] =
{
    ...
    0x09,0x21,0x11,0x01,0x00,0x01,0x22,KeyRepDescSize&0xFF,KeyRepDescSize>>8,//HID类描述符
    0x07,0x05,0x81,0x03,DEF_USB_LS_EP_SIZE,0x00,0x0a,                       //端点描述符

    0x09,0x04,0x01,0x00,0x01,0x03,0x01,0x02,0x00,                    //接口描述符,鼠标
    0x09,0x21,0x10,0x01,0x00,0x01,0x22,MOUSRepDescSize&0xFF,MOUSRepDescSize>>8,//HID类描述符
...
UINT8  MyCfgDescr_FS[ ] =
{
    ...
    0x09,0x04,0x00,0x00,0x01,0x03,0x01,0x01,0x00,                    //接口描述符,键盘
    0x09,0x21,0x11,0x01,0x00,0x01,0x22,KeyRepDescSize&0xFF,KeyRepDescSize>>8,//HID类描述符
    0x07,0x05,0x81,0x03,DEF_USB_FS_EP_SIZE,0x00,0x0a,                       //端点描述符
...
void USBDeviceInit( void )
{
    ...
    USBOTG_FS->UEP1_TX_LEN = DEF_USBD_MAX_PACK_SIZE;
    USBOTG_FS->UEP2_TX_LEN = DEF_USBD_MAX_PACK_SIZE;
    USBOTG_FS->UEP3_TX_LEN = DEF_USBD_MAX_PACK_SIZE;
    USBOTG_FS->UEP4_TX_LEN = DEF_USBD_MAX_PACK_SIZE;
    USBOTG_FS->UEP5_TX_LEN = DEF_USBD_MAX_PACK_SIZE;
    USBOTG_FS->UEP6_TX_LEN = DEF_USBD_MAX_PACK_SIZE;
    USBOTG_FS->UEP7_TX_LEN = DEF_USBD_MAX_PACK_SIZE;
                        ...
                        case USB_GET_DESCRIPTOR:
                        {
                            switch( ((pSetupReqPakHD->wValue)>>8) )
                            {
                                ...
                                case USB_DESCR_TYP_REPORT:
                                    /* USB设备报告描述符 */
                                    if( (UINT8)((pSetupReqPakHD->wIndex)&0xFF) == 0 )
                                    {
                                        pDescr = KeyRepDesc;
                                        len = sizeof( KeyRepDesc );
                                    }
                                    else if( (UINT8)((pSetupReqPakHD->wIndex)&0xFF) == 1 )
                                    {
                                        pDescr = MouseRepDesc;
